...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f1mX = new float[min];
            this.f2mY = new float[min];
            ...
            if (((double) Math.abs(this.f1mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f2mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f1mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f2mY[i3] - 1.0f)) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f1mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb2.append(this.f1mX[0]);
            String str = ",";
            sb2.append(str);
            sb2.append(this.f2mY[0]);
            sb2.append(" end:");
            int i6 = min - 1;
            sb2.append(this.f1mX[i6]);
            sb2.append(str);
            sb2.append(this.f2mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        float[] fArr = this.f1mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f2mY[i];
        }
        float f3 = (f - fArr[i]) / f2;
        float[] fArr2 = this.f2mY;
        float f4 = fArr2[i];
